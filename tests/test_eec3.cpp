...
template <typename T>
Eigen::Matrix<T, Eigen::Dynamic, 1> brute3(
  ...
  for (std::size_t i = 0; i < n; ++i) {
    for (std::size_t j = 0; j < n; ++j) {
      ...
      for (std::size_t k = 0; k < n; ++k) {
        const uint32_t b = pre.omega_next[tensoreecs::internal::omega_next_offset(a, i, k, n)];
        if (b == tensoreecs::kInvalidBin || b >= pre.n_r) {
          continue;
        }
        const uint32_t c = pre.omega_next[tensoreecs::internal::omega_next_offset(b, j, k, n)];
        if (c == tensoreecs::kInvalidBin || c >= pre.n_r) {
          continue;
        }
        out(static_cast<Eigen::Index>(c)) +=
            w(static_cast<Eigen::Index>(i)) * w(static_cast<Eigen::Index>(j)) * w(static_cast<Eigen::Index>(k));
      }
    }
  }
  return out;
...
bool run_case() {
  ...
  const auto got = tensoreecs::compute_eec3<T, Mode>(w, pre);
  const auto ref = brute3(w, r, cfg);
...
int main() {
  ...
  if (!ok) {
    std::cerr << "test_eec3 failed\n";
    return EXIT_FAILURE;
  }
  std::cout << "test_eec3 passed\n";
  return EXIT_SUCCESS;
}
